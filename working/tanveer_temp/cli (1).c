...
void process_function(char *full_string, int clientSocket)
{
    ...
    // if the first token is "create" or "delete" or "copy" then send the full string to the server
    ...
    else if (!strcmp(token, "read") || !strcmp(token, "write") || !strcmp(token, "get_info"))
    {
        ...
        // int sspno = 8888;
        int sspno;
        // set sspno as the value received from the server
        // sscanf(buffer, "%d", &sspno);

        // iterare through the buffer and check if there is any newline
