...
int delete(char *path)
{
    ...
    // fileName[strlen(fileName) - 1] = '\0';
    // check if there is a newline character at the end of fileName. If yes, then remove it
        ...
        *lastSlash = '\0';
        ...
        if (status != -2 && status != -3)
        {
            ...
            else
            {
                ...
                sprintf(sizeAndPermissions, "%d %d", size, permissions);
...
int main()
{
    ...
    struct sockaddr_in serverAddr, clientAddr, s2Addr;
    char buffer[MAX_BUFFER_SIZE];
    char ip[16], nmip[16];
    ...
    strcat(port, " ");
    strcat(port, ip);
    //concatonate the working directory with this string as well
    strcat(port, " ");
